...
{
    vector<ofxVertex *>::iterator it = mesh->verts.begin();
    vector<ofxVertex *>::iterator eit = mesh->verts.end();
    for(; it != eit; ++it)
    {
        ofxVertex *v = (*it);
        v->calculateNormal();
...
void ofxMesh::computeNormals()
{
    vector<ofxFace *>::iterator it = faces.begin();
    vector<ofxFace *>::iterator eit = faces.end();
    for(; it != eit; ++it)
    {
        ofxFace *f = (*it);
        f->calculateCentroid();
...
void ofxMesh::calculateCentroid()
{
...
void ofxMesh::draw()
{
    ...
    for(vector<ofxFace *>::iterator fit = faces.begin(); fit != faces.end(); ++fit)
    {
        ...
        {
            ofxVertex *v = (*vit);
            ofColor color = v->getColor();
            glColor4f(color.r/255.0, color.g/255.0, color.b/255.0, color.a/255.0);
            glTexCoord2fv(v->getTexcoord().getPtr());
...
ofxVertex *ofxMesh::addVertex(ofxVertex *vert)
{
    vert->setID(verts.size()); 
    verts.push_back(vert);
    return vert;
}

ofxVertex *ofxMesh::addVertex(ofVec3f pos)
{
    ofxVertex *vert = new ofxVertex(pos, verts.size());
    verts.push_back(vert);
    return vert;
}

ofxFace *ofxMesh::addFace(int index0, int index1, int index2)
{
    ofxFace *face = new ofxFace(faces.size());
    
    ofxVertex *vertex0 = verts[index0];
    ofxVertex *vertex1 = verts[index1];
    ofxVertex *vertex2 = verts[index2];
    
    ofxEdge *edge0 = getEdge(index0, index1);
    ofxEdge *edge1 = getEdge(index1, index2);
    ofxEdge *edge2 = getEdge(index2, index0);
